...
class Vessel(DictWithAttributeAccess):
    ...
    def pressure(self, area, p_ext=0):
        return self.P_DIA + self.ALASTRUEY_BETA / self.AREA_DIA * (np.sqrt(area) - sqrt(self.AREA_DIA)) + p_ext
    ...
    def step_inner_points(self):
        ...
        b_minus = 0.5 * np.multiply(np.abs(sigma_minus),
                                    1. + 5. / 19 * (1. - SCHEME_GAMMA) * (1. - np.abs(sigma_minus)))
...
class BC(DictWithAttributeAccess):
    ...
    def compute_bc_coefs(self, model, vessel):
        ...
        alpha = sign * w
        beta = (w * (sigma * se - sign * s[0]) + (u[0] - sign * sigma * ue) - sign * vessel.DT * vessel.friction(s[0],
                                                                                                                 u[0])) \
               / (1. - sign * coef * sigma)
        return alpha, beta
    ...
    def analytic_flux(self, t):
        if self.FLUX_ID == 0:
            ...
            return 10e5 * (7.9853e-06 \
                           + 2.6617e-05 * sin(2 * pi * t / T + 0.29498) + 2.3616e-05 * sin(
                        4 * pi * t / T - 1.1403) - 1.9016e-05 * sin(6 * pi * t / T + 0.40435) \
                           - 8.5899e-06 * sin(8 * pi * t / T - 1.1892) - 2.436e-06 * sin(
                        10 * pi * t / T - 1.4918) + 1.4905e-06 * sin(12 * pi * t / T + 1.0536) \
                           + 1.3581e-06 * sin(14 * pi * t / T - 0.47666) - 6.3031e-07 * sin(
                        16 * pi * t / T + 0.93768) - 4.5335e-07 * sin(18 * pi * t / T - 0.79472) \
                           - 4.5184e-07 * sin(20 * pi * t / T - 1.4095) - 5.6583e-07 * sin(
                        22 * pi * t / T - 1.3629) + 4.9522e-07 * sin(24 * pi * t / T + 0.52495) \
                           + 1.3049e-07 * sin(26 * pi * t / T - 0.97261) - 4.1072e-08 * sin(
                        28 * pi * t / T - 0.15685) - 2.4182e-07 * sin(30 * pi * t / T - 1.4052) \
                           - 6.6217e-08 * sin(32 * pi * t / T - 1.3785) - 1.5511e-07 * sin(
                        34 * pi * t / T - 1.2927) + 2.2149e-07 * sin(36 * pi * t / T + 0.68178) \
                           + 6.7621e-08 * sin(38 * pi * t / T - 0.98825) + 1.0973e-07 * sin(
                        40 * pi * t / T + 1.4327) - 2.5559e-08 * sin(42 * pi * t / T - 1.2372) \
                           - 3.5079e-08 * sin(44 * pi * t / T + 0.2328))
        elif self.FLUX_ID == 2:
            T = self.T_PER
            return 1e6 * (
                    3.1199 + 7.7982 * sin(2 * pi * t / T + 0.5769) + 4.1228 * sin(
                4 * pi * t / T - 0.8738) - 1.0611 * sin(6 * pi * t / T + 0.7240) + 0.7605 * sin(8 * pi * t / T - 0.6387)
                    - 0.9148 * sin(10 * pi * t / T + 1.1598) + 0.4924 * sin(12 * pi * t / T - 1.0905) - 0.5580 * sin(
                14 * pi * t / T + 1.042) + 0.3280 * sin(16 * pi * t / T - 0.5570)
                    - 0.3941 * sin(18 * pi * t / T + 1.2685) + 0.2833 * sin(20 * pi * t / T + 0.6702) + 0.2272 * sin(
                22 * pi * t / T - 1.4983) + 0.2249 * sin(24 * pi * t / T + 0.9924)
                    + 0.2589 * sin(26 * pi * t / T - 1.5616) - 0.1460 * sin(28 * pi * t / T - 1.3106) + 0.2141 * sin(
                30 * pi * t / T - 1.1306) - 0.1253 * sin(32 * pi * t / T + 0.1552)
                    + 0.1321 * sin(34 * pi * t / T - 1.5595) - 0.1399 * sin(36 * pi * t / T + 0.4223) - 0.0324 * sin(
                38 * pi * t / T + 0.7811) - 0.1211 * sin(40 * pi * t / T + 1.0729)
            ) / 1000 / 60
        elif self.FLUX_ID == 3:
            t += 0.055;
            T = self.T_PER
            ...
            # flux in ml/s:
            return 6.5 + 3.294 * sin(2 * pi * t / T - 0.023974) + 1.9262 * sin(4 * pi * t / T - 1.1801) - 1.4219 * sin(
                6 * pi * t / T + 0.92701)
            -0.66627 * sin(8 * pi * t / T - 0.24118) - 0.33933 * sin(10 * pi * t / T - 0.27471) - 0.37914 * sin(
                12 * pi * t / T - 1.0557)
            +0.22396 * sin(14 * pi * t / T + 1.22) + 0.1507 * sin(16 * pi * t / T + 1.0984) + 0.18735 * sin(
                18 * pi * t / T + 0.067483)
            +0.038625 * sin(20 * pi * t / T + 0.22262) + 0.012643 * sin(22 * pi * t / T - 0.10093) - 0.0042453 * sin(
                24 * pi * t / T - 1.1044)
            -0.012781 * sin(26 * pi * t / T - 1.3739) + 0.014805 * sin(28 * pi * t / T + 1.2797) + 0.012249 * sin(
                30 * pi * t / T + 0.80827)
            +0.0076502 * sin(32 * pi * t / T + 0.40757) + 0.0030692 * sin(34 * pi * t / T + 0.195) - 0.0012271 * sin(
                36 * pi * t / T - 1.1371)
            -0.0042581 * sin(38 * pi * t / T - 0.92102) - 0.0069785 * sin(40 * pi * t / T - 1.2364) + 0.0085652 * sin(
                42 * pi * t / T + 1.4539)
            +0.0081881 * sin(44 * pi * t / T + 0.89599) + 0.0056549 * sin(46 * pi * t / T + 0.17623) + 0.0026358 * sin(
                48 * pi * t / T - 1.3003)
    ...
    def compute_junction_bc_bernoulli(self, model):
        ...
        while True:
            # COMPUTE RESIDUAL
            rnorm = 0.
            P_RHO = np.array([model.vessels[self.indices[i]].pressure(AREA[i]) for i in range(N)]) + RHO / 2 * np.power(
                alphas * AREA + betas, 2)
            ...
            # FILL ROW i
            for i in range(1, N):
                JJ[i][0] = model.vessels[self.indices[0]].pressure_deriv(AREA[0]) + RHO / 2 * (
                            2 * alphas[0] * AREA[0] + betas[0])
                JJ[i][i] = -(model.vessels[self.indices[i]].pressure_deriv(AREA[i]) + RHO / 2 * (
                            2 * alphas[i] * AREA[i] + betas[i]))
            if np.any(np.isnan(RESID)) or np.any(np.isinf(RESID)):
                input(
                    f'T {model.T} bad values: inf {np.any(np.isinf(RESID))} nan {np.any(np.isnan(RESID))} area {AREA}')
            # SOLVE SYSTEM
    ...
    def run(self):
        ...
        while self.T < self.T_FINAL - 1.0e-12:
            ...
            if abs(self.T - (cnt + 1) * T_cnt) < 0.1 * self.DT:
                ...
                for i in range(3):
                    if (i == 0 and self.T >= 0.27 and self.T <= 1.4):
                        # идея: добавлять не давление, а менять поперечное сечение также синусом. Но возникла проблема, что ридиус сечения постоянный и от этого добавляется постоянное давление. Нужно вписать коэфициент, зависящий от времени и нормированный на, возможно, 1. 1-0.5*np.sin(np.pi/(1.3-0.27)*self.vessels[i].S[imid[i]] + (self.T-0.27)/1.3)
                        Pmid[i][cnt] = self.vessels[i].pressure(self.vessels[i].S[imid[i]], 100 * (
                                    np.sin(2 * np.pi / (1.4 - 0.27) * (self.T - np.pi / 4 + 0.27)) + 1))
                        Pmid[3 + i][cnt] = self.vessels[i].pressure(self.vessels[i].S[imid[i]])
                        # Pmid[3 + i][cnt] = self.vessels[i].U[imid[i]] * self.vessels[i].S[imid[i]] # Скорость
                    else:
        ...
        print(f'Pavg: {Pavg} dP {Pavg[2] - Pavg[0]} FFR = Pavg2/Pavg0 = {Pavg[2] / Pavg[0]}')
        # for i in range(len(self.vessels)):
        # 	print(f'vessel {i} times_inner_points: {self.vessels[i].tinn}')
        plt.title("Pressure vs time with p_ext")
        plt.plot(x, Pmid[0], label='Pmid_vessel_0 + p_ext', color='r')
        plt.plot(x, Pmid[1], label='Pmid_vessel_1 + p_ext', color='g')
        plt.plot(x, Pmid[2], label='Pmid_vessel_2 + p_ext', color='b')
        plt.ylabel('Pressere, mmHg')
        plt.xlabel('Time, s')
        plt.grid(visible=True)
        plt.show()

        plt.title("Pressure vs time without p_ext")
